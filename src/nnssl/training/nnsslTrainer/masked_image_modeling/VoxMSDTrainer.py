...
def select_from_pyramid(
        ...
        for i,x in enumerate(feature_pyramid):
            ...
            scale_feature = x[feature_indices[0],feature_indices[1],feature_indices[2]] 
...
class VoxMSDTrainer(AbstractBaseTrainer):
    ...
    def initialize(self):
        ...
        super(VoxMSDTrainer, self).initialize()
    ...
    def get_dataloaders(self):
        ...
        tr_transforms = self.get_training_transforms(
            ...
            3,
            1,
            self.config_plan.use_mask_for_norm,
        )
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        mask_s = mask_s.to(self.device, non_blocking=True)
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == "cuda" else dummy_context():
            ...
            if(torch.isnan(dino_loss)):
                print("Dino Loss = nan, skip") # failed to find out why... QAQ
                dino_loss = 10.0
...
class VoxMSDTrainerALPHA05(VoxMSDTrainer):
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == "cuda" else dummy_context():
            ...
            # del data
            if(torch.isnan(dino_loss)):
                print("Dino Loss出现了nan,dino loss被转为标量4.0")
                return {"loss": 1.0}
            l = rec_loss + 0.5 * dino_loss
...
class VoxMSDTrainerALPHA1(VoxMSDTrainer):
...
class VoxMSDTrainerVoxNum100(VoxMSDTrainer):
...
class VoxMSDTrainerGlobal(VoxMSDTrainer):
    ...
    def initialize(self):
        # self.recon_dataloader = self.get_qual_recon_dataloader()
        super(VoxMSDTrainerGlobal, self).initialize()
        embed_dim = 320
        proj_dim = 512
        ...
        self.proj_head_global.to(self.device)
        # self.num_iterations_per_epoch = 1
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == "cuda" else dummy_context():
            ...
            proj_feat_global_s = self.proj_head_global(feat_s[-1].view(1, 320, -1).mean(dim=-1))
            proj_feat_global_t = self.proj_head_global(feat_t[-1].view(1, 320, -1).mean(dim=-1))
            
            dino_loss = self.dino_loss(proj_feat_s, proj_feat_t, self.current_epoch)
            dino_loss_global = self.dino_loss(proj_feat_global_s, proj_feat_global_t, self.current_epoch)
            rec_loss = self.loss(output_s, label_s, mask_s)
            # del data
            if(torch.isnan(dino_loss) or torch.isnan(dino_loss_global)):
                print("Dino Loss出现了nan,dino loss被转为标量4.0")
                return {"loss": 1.0}
            l = rec_loss + 0.1*dino_loss +0.1*dino_loss_global

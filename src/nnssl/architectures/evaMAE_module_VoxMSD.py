...
from nnssl.architectures.eva import Eva
...
class EvaMAE(nn.Module):
    ...
    def forward(self, x):
        ...
        enc_feat, encoded, keep_indices = self.eva(x)
        ...
        if self.use_decoder:
            ...
            dec_feat, decoded, _ = self.decoder(restored_x)
        else:
            decoded = encoded
        ...
        if self.use_decoder:
            return enc_feat, decoded, keep_indices
        return decoded

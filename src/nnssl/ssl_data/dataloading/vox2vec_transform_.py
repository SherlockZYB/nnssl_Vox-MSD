...
class Vox2VecTransform(AbstractTransform):
    ...
    def __call__(self, **data_dict):
        ...
        new_data_dict = {
            "aug_patch_A": aug_patch_A,
            "patch_A": patch_A,
            "aug_patch_B": aug_patch_B,
            "patch_B": patch_B,
            "voxel_A":voxels_A,
            "voxel_B":voxels_B,
        }
    ...
    def get_global_and_local_crops(self, imgs: np.ndarray):
        ...
        for i in range(batch_size):
            ...
            indices = np.zeros(1)
            wait_n=0
            while len(indices) < max_num_voxels:
                wait_n += 1
                if(wait_n==10):
                ...
                if(~valid.any() or len(np.where(valid)[0])<max_num_voxels):
                    continue
                indices = np.random.choice(np.where(valid)[0], max_num_voxels, replace=False)
